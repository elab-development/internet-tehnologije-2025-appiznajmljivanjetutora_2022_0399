...
export default function AdminReviewsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await fetch("/api/recenzije/moderacija");
        const data = await res.json();
  ...
  async function deleteReview(id: number) {
    const ok = window.confirm("Da li ste sigurni da zelite da obrisete recenziju?");
    if (!ok) return;
    setError(null);
    setDeletingId(id);
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setError(data?.error || "Greska pri brisanju recenzije.");
        return;
      }
      setRows((prev) => prev.filter((r) => r.recenzijaId !== id));
    } finally {
      setDeletingId(null);
    }
  }
    ...
    <main className="relative min-h-screen overflow-hidden bg-[radial-gradient(1200px_circle_at_top,_var(--tw-gradient-stops))] from-blue-50 via-white to-sky-50 px-6 py-12">
      ...
      <div className="mx-auto max-w-5xl rounded-2xl border border-slate-200 bg-white/80 p-8 shadow-sm backdrop-blur">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Pregled svih recenzija u sistemu i mogucnost brisanja.
        </p>
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="text-sm font-semibold text-slate-900">
                      {r.ucenikIme} {r.ucenikPrezime} → {r.tutorIme} {r.tutorPrezime}
                    </div>
                  ...
                  <p className="mt-2 text-sm text-slate-700">
                    {r.komentar ? r.komentar : "Bez komentara."}
                  </p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Button
                      variant="danger"
                      size="sm"
                      disabled={deletingId === r.recenzijaId}
                      onClick={() => deleteReview(r.recenzijaId)}
                    >
                      Obrisi
                    </Button>
                  </div>
