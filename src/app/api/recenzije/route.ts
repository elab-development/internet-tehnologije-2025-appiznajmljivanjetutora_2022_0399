...
export async function POST(req: Request) {
  ...
  //ako pokusa da ostavi recenziju bez potrebnih podataka (rezervacijaId i ocena)
  ...
  if (auth.role === "UCENIK" && rezervacija.ucenikId !== auth.korisnikId) {
    return NextResponse.json({ error: "Nemate pravo da za ovu rezervaciju ostavite recenziju." }, { status: 403 });
  }
  ...
  //vrati tutora za dati termin da bi mogao da se izračuna nova prosečna ocena
  const termin = await db.query.termin.findFirst({
    where: eq(schema.termin.terminId, rezervacija.terminId),
    columns: { tutorId: true },
  });
  //ako termin ima tutora, izračunaj novu prosečnu ocenu
  if (termin?.tutorId) {
    ...
    //azuriraj prosečnu ocenu tutora u bazi
    await db
      .update(schema.tutor)
